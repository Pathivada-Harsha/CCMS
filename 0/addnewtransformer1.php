...
if (isset($_GET['new_Main_Supply_Device'])) {
    ...
    if (empty($maindevice_name)) {
        ...
        $sql = "SELECT COUNT(main_device_id) as count FROM $dbname1.Main_supply_devices WHERE main_device_id = ?";
    ...
    if ($request && isset($request['devices']) && is_array($request['devices'])) {
        ...
        if (isset($request['type']) && $request['type'] === 'switch_devices') {
            ...
            foreach ($devices as $device) {
                ...
                if (!empty($deviceId)) {
                    ...
                    if (!empty($singlephase)) {
                        $sql = "SELECT COUNT(switched_device_id) as count FROM $dbname1.Switched_devices
                ...
                if (empty($Err)) {
                    ...
                    if (!empty($singlephase)) {
                         ...
                         $sql = "INSERT INTO $dbname1.Switched_devices (main_device_id, floor_device_id,switched_device_id,phase,switched_device_name,role,login_id)
            ...
            foreach ($devices as $device) {
                ...
                if (!empty($deviceId)) {
                    $sql = "SELECT COUNT(floor_device_id) as count FROM $dbname1.floor_devices 
                ...
                if (!empty($newFloorName)) {
                    $sql = "SELECT COUNT(floor_name) as count FROM $dbname1.floor_devices WHERE floor_name=  '$newFloorName' and main_device_id = '$mainSupplyDevice' ";
                ...
                if (empty($Err)) {
                    $sql = "";
                    if (!empty($singlephase)) {
                        $sql = "INSERT INTO $dbname1.floor_devices (main_device_id, floor_name, phase, floor_device_id,floor_device_name,role,login_id) 
                    ...
                    if (!$conn->query($sql)) {
                        ...
                        $sql = "INSERT INTO $dbname2.floor_devices_data_table (device_id , kw_5_avg , kw_10_avg , kw_15_avg , kw_1_avg , kva_5_avg , kva_10_avg , kva_15_avg , kva_1_avg , kw_ph1 , kw_ph2 , kw_ph3 , kva_ph1 , kva_ph2 , kva_ph3 , pf_total , pf_5mins , pf_10mins , pf_15mins , lag_kvah_ph1 , lag_kvah_ph2 , lag_kvah_ph3 , lag_kvah_total , lead_kvah_ph1 , lead_kvah_ph2 , lead_kvah_ph3 , lead_kvah_total , lag_kwh_ph1 , lag_kwh_ph2 , lag_kwh_ph3 , lag_kwh_total , lead_kwh_ph1 , lead_kwh_ph2 , lead_kwh_ph3 , lead_kwh_total , onboard_temp , voltage_ph1 , voltage_ph2 , voltage_ph3 , current_ph1 , current_ph2 , current_ph3 , neutral_current , energy_kwh_ph1 , energy_kwh_ph2 , energy_kwh_ph3 , energy_kwh_total , energy_kvah_ph1 , energy_kvah_ph2 , energy_kvah_ph3 , energy_kvah_total , lag_kvarh_ph1 , lag_kvarh_ph2 , lag_kvarh_ph3 , lag_kvarh_total , lead_kvarh_ph1 , lead_kvarh_ph2 , lead_kvarh_ph3 , lead_kvarh_total , frequency_ph1 , frequency_ph2 , frequency_ph3 , powerfactor_ph1 , powerfactor_ph2 , powerfactor_ph3 , load_factor , load_factor_ph1 , load_factor_ph2 , load_factor_ph3 , location , signal_level , Date_Time)
                             VALUES (?, " . implode(',', $default_values) . ", ?)
                             ON DUPLICATE KEY UPDATE 
                             kw_5_avg = VALUES(kw_5_avg), kw_10_avg = VALUES(kw_10_avg), kw_15_avg = VALUES(kw_15_avg), kw_1_avg = VALUES(kw_1_avg),
                             kva_5_avg = VALUES(kva_5_avg), kva_10_avg = VALUES(kva_10_avg), kva_15_avg = VALUES(kva_15_avg), kva_1_avg = VALUES(kva_1_avg),
...
if (isset($_GET['fetch_data'])) {
    ...
    $sql_mainSupplyDevice = "SELECT main_device_id FROM $dbname1.main_supply_devices";
...
if (isset($_GET['get_floor_devices']) && isset($_GET['main_device_id'])) {
    ...
    $sql_floors = "SELECT floor_device_id FROM $dbname1.floor_devices WHERE main_device_id= '$main_device_id'";
        ...
        <div class="container-fluid">
            <div class="row d-flex align-items-center">
                <div class="col-12 p-0">
                    <p class="m-0 p-0">Pages / </span><span>Add New Connection</span></p>
                </div>
            </div>
                ...
                <div class="container mt-2">
                    ...
                    <ul class="nav nav-tabs" id="myTab" role="tablist">
                        <li class="nav-item" role="presentation">
                            <button class="nav-link active" id="main-device" data-bs-toggle="tab"
                                data-bs-target="#maindeviceid" type="button" role="tab" aria-controls="maindeviceid"
                                aria-selected="true">New Main Supply device</button>
                        </li>
                        
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="floor-device" data-bs-toggle="tab"
                                data-bs-target="#New-floor-device" type="button" role="tab" aria-controls="floor-device"
                                aria-selected="false">Add New Floor Device</button>
                        </li>
                        <li class="nav-item" role="presentation">
                            <button class="nav-link" id="switched-device" data-bs-toggle="tab"
                                data-bs-target="#New-switched-device" type="button" role="tab"
                                aria-controls="switched-device" aria-selected="false">Add New Switched Device</button>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content mt-3" id="myTabContent">
                        ...
                        <!-- New Switched Devices -->
                        <?php
                        include(BASE_PATH . "BEMS/addDevices/main-supply-device.php");
                        include(BASE_PATH . "BEMS\addDevices\add_floor_tab.php");
                        include(BASE_PATH . "BEMS\addDevices\add_building_tab.php");
                        include(BASE_PATH . "BEMS\addDevices\add_switched_tab.php");
