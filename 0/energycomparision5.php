...
$main_conn = new mysqli($main_db_config["servername"], $main_db_config["username"], $main_db_config["password"], $main_db_config["dbname"]);

if ($main_conn->connect_error) {
    die("Connection failed: " . $main_conn->connect_error);
}

// Function to get main supply devices
function getMainSupplyDevices($conn) {
    $sql = "SELECT * FROM main_supply_devices";
...
// Function to get floor devices for a main supply device
function getFloorDevices($conn, $main_device_id) {
...
// Function to get switched devices for a floor device
function getSwitchedDevices($conn, $floor_device_id) {
    $sql = "SELECT * FROM switched_devices WHERE floor_device_id = ?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("s", $floor_device_id);
    $stmt->execute();
    $result = $stmt->get_result();
    $devices = [];
...
// Function to get live data for a device from its individual database
function getLiveData($device_id, $start_date, $end_date) {
    $device_db_config = [
        "servername" => "localhost",
        "username" => "root",
        "password" => "123456",
        "dbname" => "device_" . $device_id // Assuming the database name is prefixed with "device_"
    ];

    $device_conn = new mysqli($device_db_config["servername"], $device_db_config["username"], $device_db_config["password"], $device_db_config["dbname"]);

    if ($device_conn->connect_error) {
        die("Connection failed: " . $device_conn->connect_error);
    }

    $sql = "SELECT * FROM live_data WHERE date_time BETWEEN ? AND ? ORDER BY date_time";
    $stmt = $device_conn->prepare($sql);
...
// Function to calculate energy consumption for a period
function calculateEnergyConsumption($live_data) {
    $total_energy = 0;
    foreach ($live_data as $data) {
        $total_energy += $data['energy_kwh_total'];
    }
    return $total_energy;
}

// Get the selected main supply device from the POST request
$selected_main_device = $_POST['selected_main_device'] ?? null;

if ($selected_main_device) {
    // Get data for the selected main supply device
    $main_device = getMainSupplyDevices($main_conn);
    $floor_devices = getFloorDevices($main_conn, $selected_main_device);
    
    $energy_data = [
        'lastDay' => ['current' => [], 'previous' => []],
        'lastWeek' => ['current' => [], 'previous' => []],
        'lastMonth' => ['current' => [], 'previous' => []]
    ];
    ...
    // Calculate energy consumption for main supply device
    $main_live_data_current = getLiveData($selected_main_device, $yesterday, $current_date);
    $main_live_data_previous = getLiveData($selected_main_device, date('Y-m-d H:i:s', strtotime('-2 days')), $yesterday);
    
    $energy_data['lastDay']['current']['mainSupply'] = calculateEnergyConsumption($main_live_data_current);
    $energy_data['lastDay']['previous']['mainSupply'] = calculateEnergyConsumption($main_live_data_previous);
    
    // Calculate energy consumption for floor devices and switched devices
    $floor_energy_current = 0;
    $floor_energy_previous = 0;
    $switched_energy_current = 0;
    $switched_energy_previous = 0;
    
    foreach ($floor_devices as $floor_device) {
        $floor_live_data_current = getLiveData($floor_device['floor_device_id'], $yesterday, $current_date);
        $floor_live_data_previous = getLiveData($floor_device['floor_device_id'], date('Y-m-d H:i:s', strtotime('-2 days')), $yesterday);
        
        $floor_energy_current += calculateEnergyConsumption($floor_live_data_current);
        $floor_energy_previous += calculateEnergyConsumption($floor_live_data_previous);
        
        $switched_devices = getSwitchedDevices($main_conn, $floor_device['floor_device_id']);
        
        foreach ($switched_devices as $switched_device) {
            $switched_live_data_current = getLiveData($switched_device['switched_device_id'], $yesterday, $current_date);
            $switched_live_data_previous = getLiveData($switched_device['switched_device_id'], date('Y-m-d H:i:s', strtotime('-2 days')), $yesterday);
            
            $switched_energy_current += calculateEnergyConsumption($switched_live_data_current);
            $switched_energy_previous += calculateEnergyConsumption($switched_live_data_previous);
        }
    }
    
    $energy_data['lastDay']['current']['floorDevices'] = $floor_energy_current;
    $energy_data['lastDay']['previous']['floorDevices'] = $floor_energy_previous;
    $energy_data['lastDay']['current']['switchedDevices'] = $switched_energy_current;
    $energy_data['lastDay']['previous']['switchedDevices'] = $switched_energy_previous;
    
    // Calculate energy consumption for last week
    $main_live_data_current = getLiveData($selected_main_device, $last_week, $current_date);
    $main_live_data_previous = getLiveData($selected_main_device, date('Y-m-d H:i:s', strtotime('-2 weeks')), $last_week);
    
    $energy_data['lastWeek']['current']['mainSupply'] = calculateEnergyConsumption($main_live_data_current);
    $energy_data['lastWeek']['previous']['mainSupply'] = calculateEnergyConsumption($main_live_data_previous);
    ...
    foreach ($floor_devices as $floor_device) {
        $floor_live_data_current = getLiveData($floor_device['floor_device_id'], $last_week, $current_date);
        $floor_live_data_previous = getLiveData($floor_device['floor_device_id'], date('Y-m-d H:i:s', strtotime('-2 weeks')), $last_week);
        ...
        foreach ($switched_devices as $switched_device) {
            $switched_live_data_current = getLiveData($switched_device['switched_device_id'], $last_week, $current_date);
            $switched_live_data_previous = getLiveData($switched_device['switched_device_id'], date('Y-m-d H:i:s', strtotime('-2 weeks')), $last_week);
    ...
    $energy_data['lastWeek']['current']['floorDevices'] = $floor_energy_current;
    $energy_data['lastWeek']['previous']['floorDevices'] = $floor_energy_previous;
    $energy_data['lastWeek']['current']['switchedDevices'] = $switched_energy_current;
    $energy_data['lastWeek']['previous']['switchedDevices'] = $switched_energy_previous;
    
    // Calculate energy consumption for last month
    $main_live_data_current = getLiveData($selected_main_device, $last_month, $current_date);
    $main_live_data_previous = getLiveData($selected_main_device, date('Y-m-d H:i:s', strtotime('-2 months')), $last_month);
    
    $energy_data['lastMonth']['current']['mainSupply'] = calculateEnergyConsumption($main_live_data_current);
    $energy_data['lastMonth']['previous']['mainSupply'] = calculateEnergyConsumption($main_live_data_previous);
    ...
    foreach ($floor_devices as $floor_device) {
        $floor_live_data_current = getLiveData($floor_device['floor_device_id'], $last_month, $current_date);
        $floor_live_data_previous = getLiveData($floor_device['floor_device_id'], date('Y-m-d H:i:s', strtotime('-2 months')), $last_month);
        ...
        foreach ($switched_devices as $switched_device) {
            $switched_live_data_current = getLiveData($switched_device['switched_device_id'], $last_month, $current_date);
            $switched_live_data_previous = getLiveData($switched_device['switched_device_id'], date('Y-m-d H:i:s', strtotime('-2 months')), $last_month);
    ...
    $energy_data['lastMonth']['current']['floorDevices'] = $floor_energy_current;
    $energy_data['lastMonth']['previous']['floorDevices'] = $floor_energy_previous;
    $energy_data['lastMonth']['current']['switchedDevices'] = $switched_energy_current;
    $energy_data['lastMonth']['previous']['switchedDevices'] = $switched_energy_previous;
    
    // Convert the energy data to JSON for use in JavaScript
    $energy_data_json = json_encode($energy_data);
    
    // Output the JSON data
    header('Content-Type: application/json');
    echo $energy_data_json;
} else {
    // If no main device is selected, return an error message
    // header('Content-Type: application/json');
    echo json_encode(['error' => 'No main device selected']);
}
...
function fetchHistoricalData($device_id, $device_db, $start_date, $end_date)
{
    ...
    $sql = "SELECT * FROM live_data WHERE device_id = ? AND date_time BETWEEN ? AND ? ORDER BY date_time DESC LIMIT 1";
    $stmt = $conn->prepare($sql);
...
<body>
    ...
    <script>
        ...
        function updateEnergyFlow(data) {
            ...
            const mainSupplyCurrent = data.historicalData.main.current[0];
            const mainSupplyPrevious = data.historicalData.main.previous[0];
            const currentEnergy = mainSupplyCurrent?.energy_kwh_total || 'N/A';
            const previousEnergy = mainSupplyPrevious?.energy_kwh_total || 'N/A';
            ...
            Object.values(data.historicalData.floor || {}).forEach(floor => {
                floorCurrentTotal += parseFloat(floor.current[0]?.energy_kwh_total || 0);
                floorPreviousTotal += parseFloat(floor.previous[0]?.energy_kwh_total || 0);
            });
            ...
            Object.values(data.historicalData.switched || {}).forEach(switched => {
                switchedCurrentTotal += parseFloat(switched.current[0]?.energy_kwh_total || 0);
                switchedPreviousTotal += parseFloat(switched.previous[0]?.energy_kwh_total || 0);
            });
        ...
        function updateComparisonChart(data) {
            ...
            if (!mainSupplyCurrent && !mainSupplyPrevious) {
                comparisonChart.data.labels = ['No Data Available'];
                comparisonChart.data.datasets[0].data = [0];
                comparisonChart.options.plugins.tooltip = {
                    callbacks: {
                ...
                comparisonChart.data.labels = ['Previous', 'Current'];
                comparisonChart.data.datasets[0].data = [
                    mainSupplyPrevious ? parseFloat(mainSupplyPrevious.energy_kwh_total) : 0,
                    mainSupplyCurrent ? parseFloat(mainSupplyCurrent.energy_kwh_total) : 0
                ];
                comparisonChart.options.plugins.tooltip = {
                    callbacks: {
                        label: function(context) {
        ...
        function addDeviceRow(table, name, type, floor, historicalData) {
            ...
            const currentLoad = historicalData.current[0]?.energy_kwh_total || 'N/A';
            const previousLoad = historicalData.previous[0]?.energy_kwh_total || 'N/A';
        ...
        function updateFloorDistributionChart(data) {
            ...
            data.floorDevices.forEach(device => {
                ...
                floorData[device.floor_name] += parseFloat(historicalData.current[0]?.energy_kwh_total || 0);
            });
